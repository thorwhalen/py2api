...
{argname: trans_func} links on the attribute. Further, it provides the ability to condition the trans_func to be used
on the type of the argument, and the possibility of specifying a fallback trans_func. So the full trans_spec dict
looks like this:
...
the decision of what trans_func to use follows the following algorithm:
    ...
    If _argname exists and VALTYPE is listed in _valtype:
        (say it's VALTYPE_y)
        use trans_func_y
...
def _preprocess_trans_dict(trans_dict):
    ...
    if ARG not in trans_dict:
        trans_dict['_arg'] = dict()
    if JSON not in trans_dict:
...
class InputTrans(object):
    ...
    def search_trans_func_in_valtype(self, valtype, _valtype):
        return _valtype.get(valtype)

    def __call__(self, attr, request):
        input_dict = dict()
        for arg in request.args.keys():

            trans = self.attr_trans.get(ARG)
